...
var vm = new Vue({
    ...
    methods: {
        ...
        userMessage: function () {
            window.location.href="user.html?id=" + this.user.username;
        },
...
var sl = new Vue({
    ...
    created: function () {
        ...
        axios.get('http://localhost:8080/songList/' + id)
            .then(function (response) {
                console.log(response);
                sl.songList = response.data.detail.songList;
                if (sl.songList.listSong != null) {
                    sl.listSong = sl.songList.listSong;
                }
                sl.all = Math.ceil(sl.listSong.length / sl.limit);
                //vm.creator = vm.songList.user;
                console.log(sl.listSong);
            })
            .catch(function (error) {
                console.log(error);
            });
